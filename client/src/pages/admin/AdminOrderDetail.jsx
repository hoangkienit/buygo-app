...
import {
  deleteOrderForAdmin,
  getOrderForAdmin,
  markAsFailedForAdmin,
  markAsSuccessForAdmin,
} from "../../api/order.api";
...
export const AdminOrderDetail = () => {
  ...
  useEffect(() => {
    fetchOrder();
    document.title = `Đơn hàng: ${orderId}`;
  }, [orderId]);
  ...
  return (
    <div className="order-detail-container">
      ...
      <span className="tab-nav-title">
        <a href="/super-admin/orders">Danh sách đơn hàng</a> / {order?.orderId}
      </span>
      <div className="order-container">
        <div className="order-left">
          <div className="order-header-container">
            ...
            <div className="admin-action-buttons-container">
              ...
              >
                <MdDelete className="admin-action-icon" />
              </button>
              <button
                onClick={markAsFailedOrder}
                className={`admin-action-button order-failed-button ${
                  (order?.order_status === "success" ||
                    order?.order_status === "failed") &&
                  "order-disabled-button"
                }`}
              >
                <FaBan className="admin-action-icon" />
              </button>
              <button
                onClick={markAsSuccessOrder}
                className={`admin-action-button order-success-button ${
                  (order?.order_status === "success" ||
                    order?.order_status === "failed") &&
                  "order-disabled-button"
                }`}
              >
                <FaCheckCircle className="admin-action-icon" />
              </button>
            </div>
          </div>
          <OrderStepper
          ...
          <div className="user-product-container">
            <div className="user-info-container">
              <img
                className="order-detail-user-avatar"
                src={order?.userId?.profileImg}
                alt="user-avatar"
              ></img>
              <div className="name-container">
                <p className="order-detail-username">
                  {order?.userId.username}
                </p>
                <button
                  onClick={() =>
                    navigate(`/super-admin/users/view/${order?.userId?._id}`)
                  }
                  className="view-user-detail-button"
                >
                  Xem thông tin
                </button>
              </div>
            ...
            <div className="order-detail-product-container">
              ...
              <div className="order-detail-name-container">
                ...
                {order?.order_type === "topup_package" && (
                  <p className="order-attribute-text">{order_package?.name}</p>
                )}
                {(order?.order_type === "utility_account" ||
                  (order?.order_type === "game_account" &&
                    !order?.product?.isValuable)) && (
                  <>
                    <p className="order-attribute-text">
        ...
        <div className="order-right">
          <div className="order-log-container">
            ...
            <p className="order-log">
              Đơn hàng được xử lí bởi{" "}
              <span className="order-time">{order?.processed_by || ""}</span>
            </p>
            {order?.discountCode && (
              <p className="order-log">
                Đơn hàng đã sử dụng mã giảm giá{" "}
                <span className="order-time">{order?.discountCode || ""}</span>
              </p>
            )}
