...
const Header = () => {
  ...
  const [placeholderIndex, setPlaceholderIndex] = useState(0); // Character index
  const [textIndex, setTextIndex] = useState(0); // Which phrase
  ...
  const handleUpdateUserBalance = useCallback(
    async ({ newBalance }) => {
  ...
  useEffect(() => {
    ...
    const currentText = fullPlaceholder[textIndex];
    ...
    if (placeholderIndex < currentText.length) {
      ...
      // After typing is done, wait 2 seconds then move to next phrase
      setTimeout(() => {
        setPlaceholder("");
        setPlaceholderIndex(0);
        setTextIndex((prev) => (prev + 1) % fullPlaceholder.length);
      }, 2000);
    }

    return () => clearInterval(intervalId);
  }, [placeholderIndex, textIndex, fullPlaceholder]);
